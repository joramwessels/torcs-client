...
static ObstacleSensors *oppSens[BOTS];
...
static void drive(int index, tCarElt* car, tSituation *situation)
{
	...
	ss << angle;
	ss << ",";
	ss << float(car->_curLapTime);
	ss << ",";
	ss << car->race.distFromStartLine;
	ss << ",";
	ss << distRaced[index];
	ss << ",";
	ss << car->_fuel;
	...
	ss << car->race.pos;
	ss << ",";
	ss << car->_enginerpm*10;
	ss << ",";
	ss << float(car->_speed_x  * 3.6);
	ss << ",";
	ss << float(car->_speed_y  * 3.6);
	ss << ",";
	ss << float(car->_speed_z  * 3.6);
	ss << ",";
	for (int i = 0; i < 19; i++) {
		ss << trackSensorOut[i];
		ss << ",";
	...
	training_data.open("/tmp/lliaw_data.csv", std::ofstream::app);
	training_data << s;

	// stateString += SimpleParser::stringify("curLapTime", );
	// if (getDamageLimit())
